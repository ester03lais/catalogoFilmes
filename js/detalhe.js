...
async function carregarDetalhes(){
    ...
    try {
        ...
        const data = await response.json(); renderizarDetalhes(data); 
    } catch(error) { 
    ...
    function renderizarDetalhes(item) {
        ...
        detalhesContainer.innerHTML = `
            ...
            <div class="detalhe-info"> 
                <h2>${titulo}</h2> 
                <p><b>DATA:</b> ${dataLancamento || "Nao disponivel"} 
                <br> 
                <b>NOTA:</b> ${item.vote_average}<br>
                <br> 
                <b>TEMA:</b> ${item.tagline}<br><br>  
                <b>DESCRIÇÃO:</b> ${item.overview}</p>
            </div>
     </div>
    `;
}
